...
public class FileFrame extends JFrame {
    ...
    public FileFrame(String name, String pane) throws SQLException {
        ...
        constructTable();
        confirmButton.addActionListener(e -> uploadActionPerformed());
    ...
    private void uploadActionPerformed() {
        ...
        if (user.uploadFile(path, id, description)) {
            idTextField.setText("");
    ...
    private void downloadActionPerformed() {
        ...
        if (selectedRow == -1) {
            ...
            if (value == 0) {
                try {
                    String path = getSavePath(DataProcessing.searchDoc(fileId).getFilename());
                    if (path == null) {
                        return;
                    }
                    if (user.downloadFile(fileId, path)) {
                        constructTable();
